...
# --- ANSI color codes ---
class Colors:
    GREEN = '\033[92m'; RED = '\033[91m'; YELLOW = '\033[93m'; BLUE = '\033[94m'; ENDC = '\033[0m'

# --- Data Loading Class (Unchanged) ---
...
class VLMTrainer:
    def __init__(self, config):
        ...
        # --- THE FIX: Use a Fused Optimizer for better MPS compatibility ---
        self.optimizer = torch.optim.Adam(
            self.model.parameters(), 
            lr=learning_rate, 
            fused=True if self.device == 'cuda' else False # Fused is CUDA-only, but Adam is still best
        )
        
        # --- THE FIX: Initialize a Gradient Scaler for AMP ---
        self.scaler = torch.cuda.amp.GradScaler(enabled=(self.device == 'cuda')) # Only enabled for CUDA, but good practice
    ...
    def _train_one_epoch(self):
        ...
        for embeddings, prompts, labels in pbar:
            ...
            # --- THE FIX: Use Automatic Mixed Precision context manager ---
            # For MPS, this helps manage bfloat16 and float32 operations
            ...
            # Scaler is used for CUDA, but for MPS we just call loss.backward()
            if self.device == 'cuda':
                self.scaler.scale(loss).backward()
                self.scaler.step(self.optimizer)
                self.scaler.update()
            else: # For MPS and CPU
                loss.backward()
                self.optimizer.step()
    ...
    def _validate(self):
        ...
        with torch.no_grad():
            ...
            for i, (embeddings, prompts, labels) in enumerate(pbar):
                # Use autocast here as well for consistency
                with torch.autocast(device_type=self.device, dtype=torch.bfloat16):
                    loss = self.model(embeddings, prompts, labels)
